...
2. a closed action is closed in **both** projections -- the JSON and the HTTP
   policy -- so "projection says open, route answers 503" is impossible;
...
class ProjectionDeclarationTest(unittest.TestCase):
    ...
    def test_every_action_maps_to_its_own_slice(self):
        ...
        for _, slice_id, action in capability_matrix.FEATURE_ACTIONS:
            ...
            # The declaration is closed today; what it must never do is serve an
            # action other than the one this key projects.
            self.assertLessEqual(set(spec.declared_open), {action}, slice_id)

    def test_slices_start_closed_and_unaccepted(self):
        for _, slice_id, _ in capability_matrix.FEATURE_ACTIONS:
            spec = capability_matrix.slice_for(slice_id)
            self.assertEqual(spec.open, {}, slice_id)
            self.assertFalse(spec.accepted, slice_id)
            self.assertTrue(spec.implemented, slice_id)

    def test_new_actions_are_closed_with_not_accepted_reason(self):
        projection = capability_matrix.feature_action_availability()
        for key, _, _ in capability_matrix.FEATURE_ACTIONS:
            self.assertEqual(projection[key],
                             {"available": False, "reason": "not_accepted"}, key)
    ...
    def test_routes_for_closed_actions_are_closed(self):
        for _, slice_id, action in capability_matrix.FEATURE_ACTIONS:
...
def test_auth_context_exposes_feature_actions(web_app):
    """The real endpoint carries all eight keys, closed on this build.
    ...
    contract even while every action is still unopened.
    """
