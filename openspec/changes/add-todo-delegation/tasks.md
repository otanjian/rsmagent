...
- [x] 5.1 在 `channel/web/todo_handlers.py` 新增指派、转交、收回、退回四个动作的处理入口，`owner_id` 继续取自 `ctx.user_id`
- [x] 5.2 新增窄投影来源的读取入口
- [x] 5.3 在 `channel/web/route_registry.py` 注册上述路由并标注权限要求
- [x] 5.4 确认响应不暴露接收人的角色、组织或外部身份信息，错误码不区分「不存在」与「不可见」
- [x] 5.5 测试：接口层越权用例（委托人代为处理、他人事项 ID、跨租户标识）全部被拒绝

## 6. Agent 工具

- [x] 6.1 在 `agent/tools/todo/todo_tool.py` 新增 `assign` 动作，接收人以用户名传入并由服务端在同一租户内解析
- [x] 6.2 在工具描述中写明前置条件：接收人必须由用户在当前轮次明确点名；禁止扫描会话参与方、成员目录或历史推断；单次至多一个接收人
- [x] 6.3 校验 `todo.assign`，并确认模型自报的用户/租户/成员资料不被当作授权依据
- [x] 6.4 保持 recall / reject 不在工具动作集内，直接调用被拒绝
- [x] 6.5 测试：用户明确点名时可委派；未点名、无法唯一解析或要求多接收人时拒绝且不代选
- [x] 6.6 测试：材料中的指令不构成委派委托；跨租户与越权目标被拒绝

## 7. 前端工作台

- [x] 7.1 在 `channel/web/static/js/todos.js` 增加「我委派的」分区，展示当前处理人并提供收回入口
- [x] 7.2 在新建与详情中增加接收人选择，可用性依据服务端返回的 `todo.assign` 单独判定
- [x] 7.3 无 `todo.assign` 时不提供接收人选择与委派提交并说明未开放，其他既有操作保持可用
- [x] 7.4 确保「我委派的」与「我的待办」的 items/total 不混算，且不计入本人未完成总数与顶栏角标
- [x] 7.5 「我委派的」不提供代替接收人完成、取消或编辑的入口
- [x] 7.6 在 `channel/web/static/js/i18n/todos.js` 补齐简体中文、繁体中文、英文文案
- [x] 7.7 测试：转出后事项移出「我的待办」并出现在「我委派的」；收回或退回后回到「我的待办」并从接收人列表消失
- [x] 7.8 测试：角标只统计当前处理人为本人的未完成事项
