...
master 的提交已合入 rdai，但八个接口尚未被 fork 服务，当前 Web 也未接通模型目录和多节点回退链编辑。需要在保留现有租户、用户、资源授权的条件下按功能交付，避免等待全部前端模块迁移才恢复这些能力。

## What Changes

- R1：在现有 `/auth/context` 增加按动作的功能可用性投影；Web 不再请求未开放的能力。
...
- `session-context-controls`：本人业务会话的上下文用量查询、手动压缩、并发控制和 Web 入口。

### Modified Capabilities

- `database-runtime-consumers`：增加动作级能力声明与客户端同步关闭、旧客户端/服务端兼容行为。
...
- 前端：现有 Web shell 的三个独立功能模块（能力门禁、上下文、模型目录、调度历史/创建）。
...
- 分批前置：已存在的身份、租户、任务授权、配额、审计切片；开放前以本 change 的真实验收确认，不以历史 change 的归档状态代替证据。
