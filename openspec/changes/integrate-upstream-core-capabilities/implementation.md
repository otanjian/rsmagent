...
| 提交 | 内容 | 前置 | 参考工时 |
| --- | --- | --- | --- |
| P1 | 动作投影、关闭逻辑、两端缓存失效 | 现有身份与路由门禁 | 0.5–1 人日 |
| P2 | 两个上下文接口、精确 owner 校验、压缩并发、两端 UI | P1 | 1–1.5 人日 |
...
| P6 | 运行列表/详情/删除三接口、两端历史和通知 | P1、P5 | 0.5–1 人日 |
...
类型直接复用 `desktop/src/renderer/src/types.ts` 的 ContextUsage、TaskSchedule、TaskAction、SchedulerTask、SchedulerRun、SchedulerRunDetail、SchedulerInstance、TaskRecipient。不改既有字段的意义。额外字段：
...
**修改：** `auth/capability_matrix.py`、`auth/service.py::context_for_tenant`、`channel/web/route_registry.py`；`desktop/src/renderer/src/api/context.ts`、`api/client.ts`；Web `channel/web/static/js/console.js`。

**新增：** `channel/web/static/js/functional-capabilities.js`；`desktop/src/renderer/src/api/features.ts`；`tests/test_feature_action_projection.py`、`tests/test_feature_action_clients.cjs`。
...
Desktop `features.ts` 提供同名纯函数 `isFeatureAvailable(context, key): boolean`；`DesktopContext` 的 ContextSnapshot 增加 featureActions 与 featureRevision。通过已有 `request` 请求 /auth/context；身份、tenant、broker epoch、重连变化先清空再刷新。两个新字段不存 localStorage，不给 renderer 添加 token。业务方法在发请求前检查该动作，禁止自动重放失败写请求。
...
**修改：** `channel/web/web_channel.py` 导出、route_registry、`fork/authorization.py::_require_owned_session`、`agent/protocol/agent.py::compact_context`、Web chat.html/资产登记/console.js、Desktop `ContextUsagePopover.tsx`。
...
Web `RdaiFunctionalContext` 导出 `mount({root, getContext, getSession, request, t})` 返回 `refresh()/dispose()`；getSession 返回 `{agentId,sessionId,persisted}`。在 console.js 当前会话切换/销毁处 refresh/dispose；首次落库前不轮询。Desktop 同时检查 broker epoch、Agent、session，409 提示可重试，不清空历史。两端展示 estimated 标记、used/limit、压缩中禁用和失败原因。
...
**修改：** fork scheduler.py 加列表/详情/删除 handler；web_channel、route_registry；Desktop `api/client.ts`、`types.ts`、`pages/TasksPage.tsx`、`hooks/useSchedulerNotifyPoll.ts`。
...
以下在仓库根运行；新测试文件实现后才可执行。本机用 .venv/bin/python，默认 python3 已观察到启动被终止，不能把该环境问题归为业务测试通过。Desktop 依赖按已有 lockfile 安装。
...
node --test tests/test_feature_action_clients.cjs tests/test_functional_context.cjs tests/test_desktop_context_frontend.cjs
...
| 批次 | 操作 | 通过证据 |
| --- | --- | --- |
| R1 | Web、Desktop 各查询/压缩本人非默认 Agent 会话 | 用量更新、最近消息保留；非 owner 拒绝 |
...
| R2 | 运行历史、详情、删除 | 两端一致；旧无归属记录不展示；删除不删消息 |
| 两批 | 两用户同 Agent、两租户、管理员、权限撤销 | 不越权、不跨身份复用状态；权限失败阻断发布 |
| 两批 | 设置关闭变量并重启、回退前一版本 | 声明/路由/入口一致关闭；原任务/会话仍可用 |
...
实施时新增 `evidence/acceptance.md`，每批记录：代码 SHA、测试命令/计数/失败说明、两端版本、真实测试步骤与脱敏结果、迁移/重启/回退结果。新增 `evidence/source-map.md`，记录功能→上游 SHA/符号→fork 调用点→验收测试，供后续前端结构迁移复用。
...
4. P4/P6/R2 验收完成后分别开放目标/创建与历史动作，删除对应部署 deny 键并重启；刷新两端上下文。
5. 紧急回退：将受影响完整键写入 RDAI_DISABLED_ACTIONS 并重启当前版本→确认503和停止轮询→回退该批代码/客户端。保留新增表及原 runs/messages，不执行删表脚本，不逆向改写已存配置。
