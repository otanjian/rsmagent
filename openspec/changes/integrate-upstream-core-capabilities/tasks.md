...
- [x] 3.6 实现 Web UI 测试和真实会话验收，替换这两项旧缺口断言；证据通过后才开放 usage/compact。
      真实验收（`evidence/acceptance.md` §13，原始报文 `/tmp/rdai-acc/R1-EVIDENCE-V2.md`）：
      首轮 30 项 25 PASS / 5 FAIL 跨三缺陷；修 DEF-1（`cca39b28`）、写明 DEF-2 语义（`2148573e`）、
      4.5.4 改判后**复测 31 项 31 PASS / 0 FAIL**。3.6.8 的实时数字渲染已在真实浏览器里量到
      （面板显示 `12963 / 128000`、`11 条消息 · 估算值`，与服务端载荷逐项一致），
      原先的「暂无实时上下文」是驱动在欢迎页仍在时点开触发的驱动假象，已用反向对照证明。
      **故本项与 4.5 一并勾选，两个动作已翻转开放。**
...
- [x] 4.5 完成 R1 真实验收、旧版兼容、关闭重启与回退演练；记录证据后交付 R1，R2 保持关闭。
      真实部署上重跑 4.5.1–4.5.10 全绿（`evidence/acceptance.md` §13）：目录与隐藏墓碑、
      两节点回退链的读写与**跨重启持久性**、以及「被拒 + 无写入 + 客户端视为失败」的拒绝口径。
      旧版兼容、关闭重启与代码回退演练由 §10 / §11 的真实演练覆盖（关闭态 503、数据零改动；
      代码退回基线二进制后既有数据逐项不变、扩展表被旧版本正常忽略）。
      R2 交付时**未**开放，R1 复测全绿后由 task 8.5 的第二批翻转开放（两个动作）。
...
- [x] 8.7 更新 source-map 和后续前端迁移记录，运行 OpenSpec 严格校验；所有任务和真实门槛完成后再提出归档。
      → source-map 的来源映射、批次状态表与前端迁移记录已更新到终态（八动作全部已验收开放；
      `UNACCEPTED` 为空集而「空」本身被断言）；`openspec validate --strict` 通过。
      真实门槛：R1 复测 31/31（§13）、R2 32/32（§8）、8.4 回归在最终代码状态 `de30b533` 上
      **27 failed / 5902 passed，失败集合与交付提交逐行相同**（§9）、8.5 关停与投影一致（§10）、
      8.6 回退演练（§11）、6.7 记录与访问解耦（§12）全部完成。
      **全部 41 项任务与真实门槛已完成，提请归档。**
