...
- 8 个接口有确定契约和真实授权；Web/Desktop 的支持状态一致。
...
R1 为动作投影、上下文、模型编辑器；R2 为六个调度接口和两端界面。原方案粗估 4–7 人日；细化后按 implementation.md 的 5–8.5 人日排期，已包含运行归属、压缩并发和验收。前端结构迁移继续独立跟踪。
...
客户端维护 `(身份会话, tenant_id, contextRevision)` 版本，登出、换租户和重连使版本递增；旧响应不能写入新版本。Web 接 `channel/web/static/js/console.js::_fetchTenantAuthorization`、`_invalidateAuthContext`，失效时递增序号并解除旧请求复用；Desktop 接 `desktopContext`，沿用主进程 broker，不增加 renderer token。
...
3. R1 的上下文并发保护、两端入口和模型编辑器完成，真实验收后声明相应 slice 已实现/验收并开放。
